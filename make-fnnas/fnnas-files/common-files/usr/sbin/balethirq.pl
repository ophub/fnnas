...
# RPS CPU 掩码是否排除 eth0 所占用的核心 (0: 不排除, 1: 排除)
our $rpscpu_exclude_eth0_core=1;
# RPS CPU 掩码是否排除 eth1 所占用的核心 (0: 不排除, 1: 排除)
...
############################## sub functions #########################
# 读取 /etc/balance_irq 或 /etc/config/balance_irq 中的 IRQ 均衡配置
sub read_config {
...
sub read_irq_data {
	...
	if(exists $local_map{$name}) {
	    # R68S 有 2 条 eth0 和 2 条 eth1 中断记录，仅保留第 1 条
	    next;
	} else {
	    $local_map{$name} = 1;
	}
...
sub enable_eth_rps_rfs {
	...
	# RPS 优化仅针对单队列网卡
	# 若存在 rx-1 目录，则表示该网卡支持 RSS 多队列，无需优化
	    ...
	    if( ($eth eq "eth1") && ($usb_as_eth1 == 1) ) {
                # USB 外接网卡 eth1 (RTL8153): 经测试最佳 rx_ring 范围为 100-500, 默认值 100, 超过 500 后多核负载会失衡
